...
class MSALAuthentication {
    ...
    private static let kClientId = "ceaf2001-c2b6-4212-8b6a-ce67407af53e"

    // 'Tenant ID' of your Azure AD instance - this value is a GUID
    private static let kTenantId = "2e98ed55-402e-46b9-bb19-2fd8184c8256"
    
    private static let kAuthority = try! MSALB2CAuthority(url: URL(string: "https://login.microsoftonline.com/\(kTenantId)")!)
    private static let kConfig = MSALPublicClientApplicationConfig(clientId: kClientId, redirectUri: "msauth.com.jimin.Patesian://auth", authority: kAuthority)
    ...
    public static func signin(completion: @escaping (String?, Bool?, Date?) -> Void) {
        ...
        kMSALClient.getCurrentAccount(with: msalParams) { (currentAccount, _, error) in
            if let errorDescription = error?.localizedDescription {
                print("Here and \(errorDescription)")
                return
            }
            guard let account = currentAccount else { return }

            let silentParameters = MSALSilentTokenParameters(scopes: ["User.Read Calendars.Read Calendars.ReadBasic"], account: account)
        ...
        if cachedAccessToken != nil {
            ...
            let interactiveParameters = MSALInteractiveTokenParameters(scopes: ["User.Read Calendars.Read Calendars.ReadBasic"], webviewParameters: webviewParameters)
